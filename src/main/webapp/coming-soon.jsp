			...
			<div class="flex-1 p-8">
				...
				<nav class="text-gray-500 text-sm mb-6" aria-label="Breadcrumb">
					<ol class="list-none p-0 inline-flex">
						...
						<li class="flex items-center text-gray-900 font-semibold">ComingSoon</li>
					</ol>
				</nav>
				<h2 class="text-2xl font-bold mb-6 text-red-900">Coming Soon</h2>
				...
				<div
					...
					for (Movies m : moviesList) {
						...
						if (i < movieTheatersList.size() - 1)
							movieTheaters += ",";
							}
						}

						String movieCategory = m.getGenres() != null && !m.getGenres().isEmpty()
						? m.getGenres().split(",")[0].trim().toLowerCase()
						: "";
					%>
					<div
						class="movie-card group relative overflow-hidden rounded-xl shadow-md border border-gray-200"
						...
						<div
							class="absolute bottom-0 left-0 w-full bg-black/60 p-4 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
							<div class="text-white">
								...
								<table class="text-xs w-full mb-2">
									<tbody>
										<tr>
											<td class="font-semibold w-20">Status:</td>
											<td><%=status%></td>
										</tr>
										<tr>
											<td class="font-semibold">Duration:</td>
											<td><%=m.getDuration() != null ? m.getDuration() : "N/A"%></td>
										</tr>
										<tr>
											<td class="font-semibold">Director:</td>
											<td><%=m.getDirector() != null ? m.getDirector() : "N/A"%></td>
										</tr>
										<tr>
											<td class="font-semibold">Genres:</td>
											<td><%=m.getGenres() != null ? m.getGenres() : "N/A"%></td>
										</tr>
									</tbody>
								</table>
								...
								%>
								<span
									class="inline-block mt-1 px-2 py-1 text-xs font-semibold rounded-full uppercase <%=statusBadgeClass%>"><%=status%></span>
								...
								<div class="flex flex-wrap gap-2 mt-3">
									...
									<form
										...
										<input type="hidden" name="movie_id"
											value="<%=m.getMovie_id()%>" />
										<button type="submit"
											class="px-3 py-1 bg-red-600 text-white text-xs rounded-md hover:bg-red-700 transition">Delete</button>
									</form>
									<%
									}
									%>
									...
									<form
										...
										method="post">
										<input type="hidden" name="movie_id"
											value="<%=m.getMovie_id()%>" /> <input type="hidden"
											name="theater_id" value="<%=theaterId%>" />
										<button type="submit"
											class="px-3 py-1 bg-yellow-500 text-white text-xs rounded-md hover:bg-yellow-600 transition">Remove</button>
									</form>
									<%
									}
									%>
									...
									<button type="button"
										class="trailer-btn px-3 py-1 border border-white text-white text-xs rounded-md hover:bg-white hover:text-red-600 transition"
	...
	<aside id="right-sidebar"
		...
		<ul class="space-y-1">
			...
			<li
				...
				class="block font-medium"><%=t.getName()%></span></li>
			<%
			}
			%>
			<%
